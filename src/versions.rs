...
pub fn default_prefix() -> Result<PathBuf> {
    ...
    Ok(
        PathBuf::from(std::env::var_os("HOME").context("Set --prefix when HOME is unavailable")?)
            .join(".local/softwarefactory"),
    )
}

const FILES: [&str; 3] = [
    "softwarefactory",
    "bridges/notion_review.py",
    "bridges/native_check.py",
];
...
fn version(value: &str) -> Result<semver::Version> {
    let parsed =
        semver::Version::parse(value).context("Use an exact version, for example 0.2.0")?;
...
fn prefix(path: &Path) -> Result<PathBuf> {
    ...
    if path.exists() {
        ...
        "Installation prefix is a dangling symlink"
    );
    Ok(prefix(path.parent().context("Invalid prefix")?)?
        .join(path.file_name().context("Invalid prefix")?))
}
    ...
    FILES
        ...
        .map(|name| {
            let path = safe_path(release, name)?;
                ...
                path.is_file(),
                "Incomplete release: {} is missing",
                path.display()
            );
            if *name == "softwarefactory" {
                ensure!(
                    setup::executable_available(&path.to_string_lossy()),
...
fn inspect(prefix: &Path, name: &str) -> Result<(BTreeMap<String, String>, String)> {
    ...
    if manifest.exists() {
        let stored: Manifest = serde_json::from_slice(&fs::read(manifest)?)?;
            ...
            stored.schema_version == 1 && stored.version == name && stored.sha256 == actual,
...
fn active(prefix: &Path) -> Result<Option<String>> {
    ...
    let metadata = match fs::symlink_metadata(&launch) {
        ...
        "Existing launcher is not a managed symlink; move it aside explicitly before updating"
    );
    ...
    let name = target
        ...
        .and_then(|s| s.to_str())
        .context("Unrecognised launcher target")?;
    ...
    // Canonicalize the chosen prefix above, but never accept an external launcher.
        ...
        fs::canonicalize(&target).context("Launcher target is missing")? == expected,
    ...
    if dir.exists() {
        for entry in fs::read_dir(dir)? {
            ...
            releases.push(Release {
                active: active_version.as_deref() == Some(&name),
                path: release_path(&prefix, &name)?,
                version: name,
                integrity,
            });
        }
...
fn current_hashes() -> Result<BTreeMap<String, String>> {
    Ok(BTreeMap::from([
        (
            FILES[0].into(),
            digest(&fs::read(std::env::current_exe()?)?),
        ),
        (
            FILES[1].into(),
            digest(include_bytes!("../bridges/notion_review.py")),
        ),
        (
            FILES[2].into(),
            digest(include_bytes!("../bridges/native_check.py")),
        ),
    ]))
}
pub fn preview(path: &Path, to: Option<&str>, activate: bool) -> Result<Plan> {
        ...
        cfg!(unix),
        "Versioned installation and switching currently require Unix (macOS or Linux)"
    );
    let prefix = prefix(path)?;
    ...
    let (fingerprints, integrity) = if install {
            ...
            to.is_none(),
            "Release {to_version} is not installed; run install using that release's binary first"
        );
        ...
        if to.is_none() {
                ...
                "Version {to_version} already contains a different build; releases are immutable, so use a new version"
            );
        }
        (hashes, integrity)
    };
    Ok(Plan {
        ...
        install,
        activate: (activate || from_version.is_none())
            && from_version.as_deref() != Some(&to_version),
        integrity,
        fingerprints,
    })
}
...
pub fn apply(plan: &Plan) -> Result<()> {
    ...
    let fresh = preview(
        ...
        &fresh == plan,
        "Installation changed after preview; preview the update again"
    );
    if plan.install {
        install(plan)?;
    }
    if plan.activate {
        activate(&plan.prefix, &plan.to_version)?;
    }
    Ok(())
}
fn install(plan: &Plan) -> Result<()> {
    let files = BTreeMap::from([
        (FILES[0].to_string(), fs::read(std::env::current_exe()?)?),
        (
            FILES[1].to_string(),
            include_bytes!("../bridges/notion_review.py").to_vec(),
        ),
        (
            FILES[2].to_string(),
            include_bytes!("../bridges/native_check.py").to_vec(),
        ),
    ]);
    install_files(&plan.prefix, &plan.to_version, &plan.fingerprints, &files)
}
fn install_files(
    prefix: &Path,
    name: &str,
    ...
    let result = (|| -> Result<()> {
        ...
        for name in FILES {
            fs::write(
                staging.join(name),
                files.get(name).context("Missing release file")?,
            )?;
        }
        #[cfg(unix)]
        {
            ...
            &hashes(&staging)? == expected,
            "Release files differ from the verified update"
        );
        ...
        for name in FILES.iter().chain(std::iter::once(&"manifest.json")) {
...
fn activate(prefix: &Path, name: &str) -> Result<()> {
    ...
    let result = (|| -> Result<()> {
        #[cfg(unix)]
        std::os::unix::fs::symlink(release_path(prefix, name)?.join("softwarefactory"), &temp)?;
