...
impl Screen {
    fn new(root: &Path) -> Self {
        ...
        Self {
            fields: vec![
                ...
                (
                    "Runtime arguments (JSON array)".into(),
                    serde_json::to_string(&p.runtime.command.args).unwrap_or_default(),
                ),
                ("Review arguments (JSON array)".into(), review_args),
                (
                    "Whole workflow time (seconds)".into(),
                    p.max_seconds.to_string(),
                ),
                (
                    "Whole workflow agent calls".into(),
                    p.max_dispatches.to_string(),
                ),
                (
                    "Command timeout (seconds)".into(),
                    p.command_timeout_seconds.to_string(),
                ),
                (
                    "Allowed paths (JSON array)".into(),
                    serde_json::to_string(&p.allowed_paths).unwrap_or_default(),
                ),
                (
                    "Notion database ID (optional)".into(),
                    p.kanban
                        .as_ref()
                        .map(|k| k.database_id.clone())
                        .unwrap_or_default(),
                ),
                (
                    "Notion data source ID (optional)".into(),
                    p.kanban
                        .as_ref()
                        .map(|k| k.data_source_id.clone())
                        .unwrap_or_default(),
                ),
                (
                    "Enable Notion board writes (yes/no)".into(),
                    if p.kanban.as_ref().is_some_and(|k| k.enabled) {
                        "yes"
                    } else {
                        "no"
                    }
                ...
                (
                    "Board property mapping JSON file (optional)".into(),
                    String::new(),
                ),
                ("Review kind (command/notion)".into(), p.review.kind),
                (
                    "Credential environment name (optional)".into(),
                    p.review.credential_env.unwrap_or_default(),
                ),
            ],
            selected: 0,
            stage: 0,
    ...
    fn profile(&self) -> Result<Profile> {
        ...
        p.runtime.command.args = serde_json::from_str(&self.fields[10].1)
            .context("Runtime arguments must be a JSON array")?;
        if let Some(command) = &mut p.review.command {
            command.args = serde_json::from_str(&self.fields[11].1)
                .context("Review arguments must be a JSON array")?;
        }
        p.max_seconds = self.fields[12]
            .1
            .parse()
            .context("Time budget must be positive seconds")?;
        p.max_dispatches = self.fields[13]
            .1
            .parse()
            .context("Agent calls must be a positive number")?;
        p.command_timeout_seconds = self.fields[14]
            .1
            .parse()
            .context("Timeout must be positive seconds")?;
        p.allowed_paths = serde_json::from_str(&self.fields[15].1)
            .context("Allowed paths must be a JSON array")?;
        ensure!(
            matches!(self.fields[18].1.as_str(), "yes" | "no"),
            "Board writes must be yes or no"
        );
        if !self.fields[16].1.trim().is_empty() || !self.fields[17].1.trim().is_empty() {
            let properties = if !self.fields[19].1.is_empty() {
                serde_json::from_slice(
                    &std::fs::read(&self.fields[19].1)
                        .context("Cannot read board property mapping")?,
                )?
            } else {
                p.kanban
                    .as_ref()
                    .map(|k| k.properties.clone())
                    .unwrap_or_else(crate::kanban::default_properties)
            };
            p.kanban = Some(crate::kanban::Config {
                enabled: self.fields[18].1 == "yes",
                database_id: self.fields[16].1.clone(),
                data_source_id: self.fields[17].1.clone(),
                properties,
            });
        } else {
            ensure!(
                self.fields[18].1 == "no",
                "Supply database and data source IDs before enabling board writes"
            );
            p.kanban = None;
        }
        p.review.kind = self.fields[20].1.clone();
        p.review.credential_env = if self.fields[21].1.is_empty() {
            None
        } else {
            Some(self.fields[21].1.clone())
        };
        p.validate()?;
        Ok(p)
    }
    fn preview(&mut self) -> Result<()> {
        ...
        self.notice = format!(
            ...
            readiness
                .blockers
                .iter()
                .map(|message| format!("Missing: {message}"))
                .chain(
                    readiness
                        .missing_optional
                        .iter()
                        .map(|message| format!("Optional: {message}"))
                )
                .collect::<Vec<_>>()
                .join("\n")
        );
